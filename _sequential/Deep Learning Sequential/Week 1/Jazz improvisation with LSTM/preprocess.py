...
def __parse_midi(data_fn, melody_part=5):
    # Parse the MIDI data for separate melody and accompaniment parts.
    midi_data = converter.parse(data_fn)
    # Get melody part, compress into single voice.
    melody_stream = midi_data[melody_part]     # For Metheny piece, Melody is Part #5.
    ...
    assert len(chords) == len(measures), f'FJE exception: {len(chords)} == {len(measures)}'
...
def get_musical_data(data_fn, melody_part):
    
    measures, chords = __parse_midi(data_fn, melody_part)
