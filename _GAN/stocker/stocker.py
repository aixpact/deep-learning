...
class Stocker():
    """Class for analyzing and predicting future stock prices
    """
...
#         ticker = 
        ...
        stock['ds'] = stock['Date']
        stock['Date'] = self.date_format(stock['Date']) # FJE format date
        ...
        self.min_price_date = self.stock[self.stock['y'] == self.min_price]['Date'].values
#         self.min_price_date = self.min_price_date[self.min_price_date.index[0]]
        
        self.max_price_date = self.stock[self.stock['y'] == self.max_price]['Date'].values
#         self.max_price_date = self.max_price_date[self.max_price_date.index[0]]
    ...
    def date_format(self, date):
        """Convert date in format helper function"""
    ...
    def make_df(self, start_date, end_date, df=None):
        """Return the dataframe trimmed to the specified date range.
        """
    ...
    def plot_stock(self, start_date=None, end_date=None, stats=['Adj. Close'], plot_type='basic'):
        ...
        for i, stat in enumerate(stats):
            ...
            date_stat_min = stock_plot[stock_plot[stat] == stat_min]['Date'].values
#             date_stat_min = date_stat_min[date_stat_min.index[0]]
            
            date_stat_max = stock_plot[stock_plot[stat] == stat_max]['Date'].values
#             date_stat_max = date_stat_max[date_stat_max.index[0]]
            
            print('Maximum {} = {:.2f} on {}.'.format(stat, stat_max, date_stat_max))f
            print('Minimum {} = {:.2f} on {}.'.format(stat, stat_min, date_stat_min))
            print('Current {} = {:.2f} on {}.\n'.format(stat, self.stock.loc[len(self.stock) - 1, stat], self.max_date))
            
            # Percentage y-axis
            if plot_type == 'pct':
                # Simple Plot 
                if stat == 'Daily Change':
                    y = 100*stock_plot[stat]
                    ylabel = 'Change (%)'
                else:
                    y = 100*(stock_plot[stat]-stat_avg)/stat_avg
                    ylabel = 'Change Relative to Average (%)'
            # Stat y-axis
        ...
        plt.plot(stock_plot['Date'], y, color=colors[i], linewidth=1.5, alpha=0.8, label=stat)
        plt.xlabel('Date')  
        plt.xlabel(ylabel)     
...
#         plt.style.use('dark_background')
    ...
    def create_prophet_model(self, days=0, resample=False):
        ...
        if days > 0:
            ...
            print('Predicted Price on {} = ${:.2f}'.format(
                future.loc[len(future) - 1, 'ds'], future.loc[len(future) - 1, 'yhat']))
    ...
    def evaluate_prediction(self, start_date=None, end_date=None, nshares=None):
        ...
        if not nshares:
            ...
            print('\nPredicted price on {} = ${:.2f}.'.format(max(future['ds']), future.loc[len(future) - 1, 'yhat']))
            print('Actual price on    {} = ${:.2f}.\n'.format(max(test['ds']), test.loc[len(test) - 1, 'y']))
